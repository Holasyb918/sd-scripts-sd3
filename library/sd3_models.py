    ...
    def forward(
        ...
        if not self.blocks_to_swap:
            for block_idx, block in enumerate(self.joint_blocks):
                ...
                if block_controlnet_hidden_states is not None:
                    x = x + block_controlnet_hidden_states[block_idx % controlnet_depth]
        else:
            for block_idx, block in enumerate(self.joint_blocks):
...
class ControlNetSD3(nn.Module):
    def __init__(
        ...
        pos_embed_latent_sizes: Optional[list[int]] = None,
        model_type: str = "sd3m",
        num_extra_conditioning_channels: int = 1,
        controlnet_depth: int = 12,
    ):
        super().__init__()
        self._model_type = model_type
        ...
        self.num_heads = num_heads
        self.num_extra_conditioning_channels = num_extra_conditioning_channels
        ...
        self.x_embedder = PatchEmbed(
            input_size,
            patch_size,
            in_channels + num_extra_conditioning_channels,
            self.hidden_size,
            bias=True,
            strict_img_size=self.pos_embed_max_size is None,
        )
        ...
        controlnet_x_embedder = PatchEmbed(
            input_size,
            patch_size,
            in_channels + num_extra_conditioning_channels,
            self.hidden_size,
        ...
        self.controlnet_x_embedder = zero_module(controlnet_x_embedder)
    ...
    def forward(
        ...
        x = self.x_embedder(x) + pos_embed
        controlnet_cond = self.controlnet_x_embedder(controlnet_cond)
        x = x + controlnet_cond
        del pos_embed
        ...
        if not self.blocks_to_swap:
            for block_idx, block in enumerate(self.joint_blocks):
                ...
                block_samples += (self.controlnet_blocks[block_idx](x),)
        else:
            for block_idx, block in enumerate(self.joint_blocks):
                self.offloader.wait_for_block(block_idx)
...
def create_sd3_controlnet(params: SD3Params, attn_mode: str = "torch") -> ControlNetSD3:
    sd3_controlnet = ControlNetSD3(
        input_size=None,
        pos_embed_max_size=params.pos_embed_max_size,
        patch_size=params.patch_size,
        in_channels=16,
        ...
        controlnet_depth=params.controlnet_depth,
    )
